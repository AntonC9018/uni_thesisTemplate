...
% Необходимо только для аннотации, для магистратуры.
\newcommand{\docTitleEng}{Thesis Name} % Doar pentru master
\newcommand{\docTypeEng}{master thesis}
\newcommand{\programulDeStudiiEng}{master}

\newcommand{\authorName}{Nume Prenume}
\newcommand{\programulDeStudiiRo}{master} % licență / master
...
\newcommand{\programulDeStudiiRu}{магистра} % бакалавриата
...
\newcommand{\conducatorTitleRu}{асистент университета} % доктор, преподаватель университета

% Если этот документ по практике, уберите % снизу.
% \newcommand{\isPractica}{}

% Игнорируются для практики:
\newcommand{\docTitleRo}{Explorarea formatului PNG}
\newcommand{\docTitleRu}{Исследование формата PNG}

% Для практики: 
% I licență - ознакомительная практика
% II licență - производственная практика
% III licență - научно-исследовательская практика
% II master - производственная практика
% курсовая / бакалаврская / магистерская (с разными склонениями)
\newcommand{\docTypeRuPredlozhny}{магистерская работа} 
\newcommand{\docTypeRuDatelny}{магистерской работе}
% Practica:
...
% de an / de licență / de master (определенный / неопределенный артикль)
...
\newcommand{\studentTypeRo}{masterand} % student / absolvent / masterand
\newcommand{\studentTypeRu}{студент} % студент / выпускник / магистрант
...
% Пояснение использования ИИ, если выбрали "assisted", на русском или на румынском
...
Когда \verb|\verb| используется для \verb|\subsection|, необходимо применить \verb|\cprotect| к этой \verb|\subsection|.
...
Таблица~\labelcref{pixel_order_table} предоставляет пример вставки таблиц с автоматической нумерацией.
...
\cprotect\section{Библиотека \verb|minted| для кода}
...
Листинг~\labelcref{code_direct_example} показывает прямую вставку блока кода.

\insertCode[code_direct_example]{cpp}{Пример прямой вставки блока кода}{
// любой код здесь
int main()
{
    std::cout << "hello";
...
Листинг~\labelcref{../src/sourcefile.zig} вставляет целый файл.

\insertCodeFile{zig}{../src/sourcefile.zig}{Полный исходный файл}

\subsection{Вставка части файла}

Листинг~\labelcref{code_range_example} вставляет только диапазон строк, переданный через опции \verb|minted|.
...
Листинг~\labelcref{code_segment_example} вставляет сегмент, выделенный комментариями в исходном файле.
...
Листинг~\labelcref{code_text_example} вставляет обычный текст как блок кода.

\insertCode[code_text_example]{text}{Многострочный текст как код}{
Regular text
Multiline
With any characters |||
}
...
Листинг~\labelcref{code_text_no_lines_example} скрывает номера строк через опции \verb|minted|.

\insertCode[code_text_no_lines_example][linenos=false]{text}{Текст без номеров строк}{
Regular text
Multiline
With any characters |||
}
...
На Рисунке~\labelcref{interface_sketch.png} представлен эскиз графического интерфейса.
...
Например, на Рисунке~\labelcref{wide.png} широкая картинка не выйдет за пределы,
а на Рисунке~\labelcref{long.png} она не уйдет за страницу.
...
а именно:
\begin{itemize}
  \item Рисунок~\labelcref{cat_1} демонстрирует кота номер 1
  \item Рисунок~\labelcref{cat_2} демонстрирует кота номер 2
\end{itemize}
...
Для прямого ненумерованного кода в приложениях можно использовать команду \verb|\insertCode*|.
...
\insertCodeFile[whole_appendix_code][linenos=false]{js}{../src/appendix_example.js}{Код, покрытый ссылкой на приложение}
