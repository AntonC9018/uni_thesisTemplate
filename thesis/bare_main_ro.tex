...
Tabelul~\labelcref{pixel_order_table} prezintă un exemplu de inserare a tabelelor cu numerotare automată.
...
Codul~\labelcref{code_direct_example} prezintă inserarea directă a unui bloc de cod.

\insertCode[code_direct_example]{cpp}{Exemplu de bloc de cod inserat direct}{
// orice cod aici
int main()
{
    std::cout << "hello";
...
Codul~\labelcref{../src/sourcefile.zig} inserează un fișier întreg.

\insertCodeFile{zig}{../src/sourcefile.zig}{Fișier sursă complet}

\subsection{Inserarea unei bucăți din fișier}

Codul~\labelcref{code_range_example} inserează numai intervalul de linii transmis prin opțiunile \verb|minted|.
...
Codul~\labelcref{code_segment_example} inserează un segment delimitat prin comentarii în fișierul sursă.
...
Codul~\labelcref{code_text_example} inserează text simplu ca bloc de cod.

\insertCode[code_text_example]{text}{Text pe mai multe rânduri inserat ca cod}{
Regular text
Multiline
With any characters |||
}
...
Codul~\labelcref{code_text_no_lines_example} ascunde numerele de rând prin opțiunile \verb|minted|.

\insertCode[code_text_no_lines_example][linenos=false]{text}{Text fără numere de rând}{
Regular text
Multiline
With any characters |||
}
...
În Figura~\labelcref{interface_sketch.png} este prezentată o schiță a interfeței grafice.
...
De exemplu, în Figura~\labelcref{wide.png} o imagine lată nu va depăși limitele paginii,
iar în Figura~\labelcref{long.png} nu va ieși în afara paginii.
...
și anume:
\begin{itemize}
  \item Figura~\labelcref{cat_1} demonstrează pisica numărul 1
  \item Figura~\labelcref{cat_2} demonstrează pisica numărul 2
\end{itemize}
...
Pentru cod direct nenumerotat în anexe se poate folosi comanda \verb|\insertCode*|.
...
\insertCodeFile[whole_appendix_code][linenos=false]{js}{../src/appendix_example.js}{Cod acoperit de referința la anexă}
