...
def test_code_environments_accept_direct_braced_body_without_percent(build):
    assert_build_succeeded(build)
    assert_pdf_contains(build, "Direct environment reference: 1.1.")
    ...
    assert_pdf_not_contains(build, "Plain unnumbered code block: Codul")
